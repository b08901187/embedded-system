...
SocketDemo::~SocketDemo()
{
    ...
    void SocketDemo::run()
    {
        ...
        if (!send_start_request(0)) {
            return;
        }

        if (!receive_http_response(0)) {
            return;
        }
        
        if (!send_start_request(1)) {
            return;
        }

        if (!receive_http_response(1)) {
            return;
        }
...
int SocketDemo::get_ref_temp()
{
    return ref_temp;
}

bool SocketDemo::get_state()
{
    return start_or_not;
}
...
{
    const char hostname[] = "192.168.43.158";
...
bool SocketDemo::send_start_request(int x)
{
    /* loop until whole request sent */
    const char buffer0[] = "What is your preferred temperature?  (20 Celsius ~ 45 Celsius)";
    const char buffer1[] = "Send any chracter to start";
    if(x == 0)
    {
        nsapi_size_t bytes_to_send = strlen(buffer0);
        nsapi_size_or_error_t bytes_sent = 0;

        printf("\r\nSending message: \r\n%s", buffer0);

        while (bytes_to_send) {
            bytes_sent = _socket.send(buffer0 + bytes_sent, bytes_to_send);
            if (bytes_sent < 0) {
    ...
    {
        nsapi_size_t bytes_to_send = strlen(buffer1);
        nsapi_size_or_error_t bytes_sent = 0;

        printf("\r\nSending message: \r\n%s", buffer1);

        while (bytes_to_send) {
            bytes_sent = _socket.send(buffer1 + bytes_sent, bytes_to_send);
            if (bytes_sent < 0) {
                printf("Error! _socket.send() returned: %d\r\n", bytes_sent);
                return false;
            } else {
...
bool SocketDemo::receive_http_response(int x )
{
    char buffer0[2];
    char buffer1[2];
    ...
    if(x == 0)
    {
        while (result > 0 && remaining_bytes > 0) {
            result = _socket.recv(buffer0 + received_bytes, remaining_bytes);
            if (result < 0) {
                printf("Error! _socket.recv() returned: %d\r\n", result);
                return false;
            }

            ref_temp = 10*(int)(buffer0[0] - 48) + (int)(buffer0[1] - 48);

            printf("reference temp is = %d\r\n",ref_temp);
        ...
        while (result > 0 && remaining_bytes > 0) {
            result = _socket.recv(buffer1 + received_bytes, remaining_bytes);
            if (result < 0) {
                printf("Error! _socket.recv() returned: %d\r\n", result);
                return false;
            }
